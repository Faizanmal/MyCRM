...
"""
    ...
    def _generate_with_rules(self, recording, full_text: str,
        ...
        from .advanced_models import MeetingActionItem, MeetingSummary
        ...
        participants = list({s.speaker for s in segments})
            ...
            for moment_type, patterns in self.moment_patterns.items():
                for pattern in patterns:
                    if re.search(pattern, text):
                        ...
                        importance = 'medium'
                        if moment_type in ['buying_signal', 'objection'] or moment_type in ['competitor_mention', 'decision_maker']:
                            importance = 'high'
