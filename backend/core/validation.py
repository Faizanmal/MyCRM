...
class PhoneValidator:
    ...
    def validate(cls, phone: str, country: str = 'US') -> ValidationResult:
        ...
        if cleaned.startswith('+'):
            if cls.PATTERNS['INTL'].match(cleaned):
...
class URLValidator:
    ...
    def validate(cls, url: str, allow_localhost: bool = False) -> ValidationResult:
        ...
        if parsed.scheme not in cls.ALLOWED_SCHEMES:
            return ValidationResult.failure([f'URL scheme must be http or https'])
...
class DataSanitizer:
    ...
    @classmethod
    def sanitize_dict(cls, data: Dict[str, Any], schema: Optional[Dict[str, Dict]] = None) -> Dict[str, Any]:
